                ...
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <select name="roomid" id="roomid" class="form-control">
                      ...
                      <?php
                        $sql = "SELECT id,room_no FROM room";
                        $query = mysqli_query($conn,$sql);
                        if($query){
                            while($row = mysqli_fetch_array($query)){
                                $id = $row["id"];
                                $name = $row["room_no"];
                                $block =$row["hostel-block"];
                                echo "
                                <option value='$id'>$name - $block</option>
                                ";
                            }
                        }
                      ?>
...
$("#photo").change(function(){
  ...
  if(!((fileType == match[0]) || (fileType == match[1]) || (fileType == match[2])) || file.size > 5000000){
    $("#photo-status").html("Ensure file type is jpeg,jpg or png and size below 5mb");
